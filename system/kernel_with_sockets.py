...
def init():
    ...
    while not sockets_ok:
        ...
        if len(errors) > 0:
            ...
            sockets_ok = True
        memory.write('sockets', modules, 'kernel', cell_name='kernel', mode='const')
...
def deinit():
    try:
        sockets = memory.read('sockets', 'kernel', cell_name='kernel')
    except MemoryError:
        raise err.DeinitError('sockets are even not initialized!')
    else:
        for each_socket in sockets:
            each_socket.close()
        print('[INFO] Sockets successfully closed')
